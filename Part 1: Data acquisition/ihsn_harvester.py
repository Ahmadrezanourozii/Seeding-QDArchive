...
Description: Implementation of the IHSN (NADA) API harvester; specialized for metadata 
             aggregation and parallelized resource inspection.
"""
...
IHSN_API_BASE = "https://catalog.ihsn.org/index.php/api"
...
class IHSNHarvester(AbstractBaseHarvester):
    """
    Harvester for the IHSN (NADA) catalog.
    
    Uses the NADA API to search for datasets and perform deep inspection of 
    resource metadata to identify relevant QDA files.
    """
    ...
    def __init__(
        ...
        *, 
        ...
        self._token = self._settings.api.get("ihsn")
    ...
    async def fetch_records(
        ...
        **kwargs: Any,
        ...
        """Harvests datasets from IHSN matching the provided search criteria."""
        ...
        async with self.create_client_session() as session:
            ...
            for q_term in search_queries:
                ...
                while True:
                    search_url = f"{IHSN_API_BASE}/catalog/search?ps={page_size}&page={page}&sk={quote(q_term)}"
                    try:
                        ...
                        for row in rows:
                            ...
                            if idno and idno not in seen_ids:
                                inspection_tasks.append(
                                    self._inspect_record(session, row, headers, is_strict_term)
                                )
                                seen_ids.add(idno)
    ...
    async def _inspect_record(
        ...
        """Performs deep inspection of a single IHSN record and its associated resources."""
        idno = row.get("idno")
        if not idno:
            return None
            
        async with self._semaphore:
            details_url = f"{IHSN_API_BASE}/catalog/{idno}"
            resources_url = f"{IHSN_API_BASE}/catalog/resources/{idno}"
            
            try:
                # Fetch detailed metadata (abstract, notes, etc.)
        ...
        # Consolidate metadata blocks for relevance scoring
        if not is_strict:
            text_blocks = [
                row.get("title", ""),
                row.get("subtitle", ""),
        ...
        record_meta = {
            ...
            "license_id": "IHSN-Standard-Terms",
            "raw": dataset,
            "persons": persons
        }
